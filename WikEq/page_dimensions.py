...
def WikEq(dimension):
    ...
    with theme.row():
            ...
            with ui.card():
                ...
                ui.label(dimj['Base_quantity'])
                ui.label(dimj['Symbol'])
                ui.label(dimj['Description'])
                ui.label(dimj['SI_base_unit'])
                ui.label(dimj['Dimension'])
                ui.link('WikiLink', dimj['WIKI_LINK'])

            # get all equations this id is found in
            eq_ids = df_eqv[df_eqv['D_Name'] == dimension]['EQ_id'].unique()
            eqs = df_eq.loc[df_eq['EQ_id'].isin(eq_ids), 'Name'].sort_values()
            with ui.card():
